...
Metrics, observations, dashboards, and immutable reporting clocks with cited explanations.

Proofline Insights is a focused, public MIT distribution for the `insights` module in [managed-oss-cloud](https://github.com/rohanarun/managed-oss-cloud). It includes a product web UI, a product-scoped HTTP client, the `proofline-insights` CLI, and a stdio MCP server exposing only this product's 9 typed actions.

![Proofline Insights sample workspace](./docs/product-workspace.png)
...
- Measurement-clock provenance
- Hypotheses separated from facts
- No autonomous alert or metric mutation
...
npm install
npm link
export PROOFLINE_INSIGHTS_TOKEN="a-scoped-workspace-token"
export PROOFLINE_INSIGHTS_URL="https://cloud.getsupers.com"
proofline-insights actions
proofline-insights workspace
proofline-insights page '{"limit":50,"state":"active","search":"Title prefix"}'
proofline-insights detail '00000000-0000-4000-8000-000000000001'
proofline-insights action source-register '{"name":"Revenue ledger","kind":"table","ownerRef":"user-owner-0001","refreshCadence":"daily","idempotencyKey":"insights.source-register.example-0001"}'
```
...
export PROOFLINE_INSIGHTS_TOKEN="a-scoped-workspace-token"
export PROOFLINE_INSIGHTS_URL="https://cloud.getsupers.com"
export PROOFLINE_INSIGHTS_WEB_KEY="a-separate-random-browser-key"
npm start
```
...
```bash
docker build -t proofline-insights:0.3.0 .
docker run --rm -p 4173:4173 \
  -e PROOFLINE_INSIGHTS_TOKEN \
  -e PROOFLINE_INSIGHTS_URL \
  -e PROOFLINE_INSIGHTS_WEB_KEY \
  proofline-insights:0.3.0
```
...
```json
{
  "mcpServers": {
    "proofline-insights": {
      "command": "proofline-insights-mcp",
      "env": {
        "PROOFLINE_INSIGHTS_TOKEN": "a-scoped-workspace-token",
        "PROOFLINE_INSIGHTS_URL": "https://cloud.getsupers.com"
      }
    }
  }
}
...
Then point `PROOFLINE_INSIGHTS_URL` at the self-hosted control-plane origin. All products may share the same backend and PostgreSQL cluster while the backend preserves workspace and module boundaries.

## Typed action catalogue

| Action ID | Capability | Token scope | Operation |
|---|---|---|---|
| `source-register` | Register governed data source | `write` | `command` |
| `metric-define` | Define typed metric | `write` | `command` |
| `observation-import` | Import approved observations | `write` | `command` |
| `dashboard-create` | Create dashboard contract | `write` | `command` |
| `chart-add` | Add deterministic chart | `write` | `command` |
| `alert-rule-create` | Create inert alert rule | `write` | `command` |
| `snapshot-freeze` | Freeze reporting snapshot | `write` | `command` |
| `anomaly-propose` | Queue cited anomaly hypotheses | `ai` | `ai` |
| `narrative-propose` | Queue dashboard narrative | `ai` | `ai` |
